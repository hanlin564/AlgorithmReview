...
### [3. 剑指offer 04.第一个只出现一次的字符](https://leetcode-cn.com/problems/di-yi-ge-zhi-chu-xian-yi-ci-de-zi-fu-lcof/)
...
### [7. 下一个排列（55）](https://leetcode-cn.com/problems/jump-game/)
...
//        这样我们就找到了缺失的数字。
        int n = nums.length;
        for (int num : nums) {
            //数可能已经被增加过，因此需要对 nn 取模来还原出它本来的值
            int x = (num - 1) % n;
            nums[x] += n;
        }
...
### [4. 合并区间（48）](https://leetcode-cn.com/problems/rotate-image/)
